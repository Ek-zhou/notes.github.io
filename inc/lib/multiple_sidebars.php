...
class sidebar_generator {
        ...
        if(is_array($sidebars)){
            foreach($sidebars as $sidebar){
                ...
                register_sidebar(array(
                    'name'=>$sidebar,
                    'id' => 'exort-sidebar-'.strtolower($sidebar_class),
        ...
        ?>
            <script>
                function add_sidebar( sidebar_name )
                {
                    ...
                    mysack.onError = function() { alert("<?php esc_html_e( 'Ajax error. Cannot add sidebar', 'exort' ); ?>")};
                    mysack.runAJAX();
                    return true;
                }
    ...
    static function add_sidebar(){
        ...
        if(isset($sidebars[$id])){
            die("alert('".__('Sidebar already exists, please use a different name.', 'exort')."')");
        }
        ...
        $js = "
            ...
            //var textNode = document.createTextNode('[<a href=\'javascript:void(0);\' onclick=\'return remove_sidebar_link($name);\'>".__( 'Remove', 'exort' )."</a>]');
            //cellLeft.appendChild(textNode)

            var cellLeft = row.insertCell(2);
            removeLink = document.createElement('a');
            linkText = document.createTextNode('".__( 'remove', 'exort' )."');
    ...
    static function remove_sidebar(){
        ...
        $id = sidebar_generator::name_to_class($name);
        if(!isset($sidebars[$id])){
            die("alert('".__( 'Sidebar does not exist.', 'exort' )."')");
        }
    ...
    static function admin_menu(){
        add_theme_page(__( 'Sidebars', 'exort' ), __( 'Sidebars', 'exort' ), 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));
    ...
    static function admin_page(){
        ...
        <script>
            function remove_sidebar_link(name,num){
                answer = confirm("<?php esc_html_e( 'Are you sure you want to remove %s?\nThis will remove any widgets you have assigned to this sidebar.', 'exort' ) ?>".replace( "%s", name ));
                if(answer){
            ...
            function add_sidebar_link(){
                var sidebar_name = prompt("<?php esc_html_e( 'Sidebar Name:', 'exort' ); ?>","");
        ...
        <div class="wrap">
            ...
            <table class="widefat page" id="sbg_table" style="width:600px;">
                <tr>
                    <th><?php esc_html_e( 'Sidebar Name', 'exort' ); ?></th>
                    <th><?php esc_html_e( 'CSS class', 'exort' ); ?></th>
                    <th><?php esc_html_e( 'Remove', 'exort' ); ?></th>
                </tr>
                <?php
                $sidebars = sidebar_generator::get_sidebars();
                ...
                <?php
                    ...
                    ?>
                    <tr>
                        <td colspan="3"><?php esc_html_e( 'No Sidebars defined', 'exort' ); ?></td>
                    </tr>
                    <?php
                }
                ?>
            ...
            <div class="add_sidebar">
                <a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php esc_attr_e( 'Add a sidebar', 'exort' ); ?>" class="button-primary"><?php esc_html_e( '+ Add New Sidebar', 'exort' ); ?></a>
    ...
    */
    static function save_form($post_id){
        ...
        if(!empty($is_saving)){
            ...
            add_post_meta($post_id, 'sbg_selected_sidebar', esc_html( $_POST['sidebar_generator'] ));
            add_post_meta($post_id, 'sbg_selected_sidebar_replacement', esc_html( $_POST['sidebar_generator_replacement']));
        }
        }
    }
    ...
    <div id='sbg-sortables' class='meta-box-sortables'>
        <div id="sbg_box" class="postbox " >
            <div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'exort' ); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e( 'Sidebar', 'exort' ); ?></span></h3>
            <div class="inside">
                <div class="sbg_container">
                    <input name="sbg_edit" type="hidden" value="sbg_edit" />

                    <p><?php esc_html_e( 'Please select the sidebar you would like to display on this page. <strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars.', 'exort' ); ?></p>
                    <ul>
                    <?php
                        ...
                        for($i=0;$i<1;$i++){ ?>
                            ...
                            <select name="sidebar_generator[<?php echo esc_attr( $i )?>]" style="display: none;">
                                <option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php esc_html_e( 'WP Default Sidebar', 'exort' ); ?></option>
                            <?php
                            ...
                            <select name="sidebar_generator_replacement[<?php echo esc_attr( $i )?>]">
                                <option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php esc_html_e( 'None', 'exort' ); ?></option>
                            <?php
